...
out.println("location.href='../login_form.jsp?url=bbs/board_rq_ulist.jsp';");
out.println("</script>");	
out.close();
}
...
String where = " WHERE br_isview = 'y' AND mi_id='" + mi_id + "' ";

try {
	stmt = conn.createStatement();
	
	sql = "SELECT COUNT(*) FROM t_board_request WHERE br_isview = 'y' AND mi_id='" + mi_id + "' ";
	...
	sql = "SELECT br_idx, br_isanswer, br_select, br_title, IF (CURDATE() = DATE(br_date), TIME(br_date), REPLACE(MID(br_date, 3, 8), '-', '.')) br_date, br_read FROM t_board_request " + where + "order by br_idx desc limit " + start + ", " + psize;
	// System.out.println(sql);
...
<table width="700" border="0" cellpadding="0" cellspacing="0" id="list" align="center">
	<tr height="30">
		<th width="10%">번호</th>
	...
	if (rs.next()) {
		...
		do {
			int titleCnt = 24;
			String reply = "",  title = rs.getString("br_title");
			
			if (title.length() > titleCnt)
				title = title.substring(0, titleCnt - 3) + "...";
			title = "<a href='board_rq_uview.jsp?idx=" + rs.getInt("br_idx") + "&cpage=" + cpage + schargs +"'>" + title + "</a>";
...
<table width="700" align="center">
	<tr>
		...
		<% 
		if (rcnt > 0) { // 게시글이 있으면
			String link = "board_rq_ulist.jsp?cpage=";
			if (cpage == 1) {
				out.println("[처음]&nbsp;&nbsp;&nbsp;[이전]&nbsp;&nbsp;");
			} else {
		...
		<td width="*">
			<input type="button" value="글등록" onclick="location.href='board_rq_form.jsp';" />
		</td>
	</tr>
</table>
