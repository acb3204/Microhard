...
if ((rpms.equals("b")) && !(isLogin) && !kind.equals("del")) {
	br_writer = request.getParameter("br_writer");
	br_pw = request.getParameter("br_pw");
	br_ismem = "n";
} else if (isLogin) {
	br_writer = loginInfo.getMi_nick();
}
...
try {
	...
	if (kind.equals("in")) {				// 댓글 등록일 경우 
		...
		if (rpms.equals("a")) {
			...
			sql = "update t_board_" + id + "_reply set br_isview = 'n' where mi_nick = '" + loginInfo.getMi_nick() + "' and br_idx = " + br_idx;
		} else if (rpms.equals("b")) {
			sql = "SELECT br_ismem FROM t_board_" + id + "_reply WHERE br_idx=" + br_idx;
			rs = stmt.executeQuery(sql);
			if (rs.next()) {
				br_ismem = rs.getString("br_ismem");
			}
			
			if (br_ismem.equals("n")) {
				out.println("<script>location.replace('board_reply_pw.jsp" + args + "&br_idx=" + br_idx + "');</script>");
				sql = "";
			} else {
				sql = "update t_board_" + id + "_list set bl_replay = bl_replay - 1 where bl_idx = " + bl_idx;
				stmt.executeUpdate(sql);
				sql = "update t_board_" + id + "_reply set br_isview = 'n' where br_writer = '" + loginInfo.getMi_nick() + "' and br_idx = " + br_idx;	
			}
		}
