...
String action = "board_form.jsp";		// 수정시 이동할 파일

if (kind.equals("del")) action = "board_proc_del.jsp?id=" + id;	// 삭제시 이동할 파일 

// 현재 비밀번호를 입력받을 게시글이 비회원 글이 맞는지 검사 하는 영역

try {
	stmt = conn.createStatement();
	sql = "select 1 from t_board_" + id + "_list where bl_isview = 'y' and bl_ismem = 'n' and bl_idx = " + idx;
	...
	out.println("게시판 비밀번호 입력 폼에서 문제가 발생했습니다.");
	e.printStackTrace();
} finally {
...
<div id="box">
	<p>비밀번호를 입력하세요.</p>
	<input type="password" name="bl_pw" style="margin-bottom:10px;" /><br />
...
<script>
document.frm.bl_pw.focus();	// 비밀번호 입력 폼
</script>
